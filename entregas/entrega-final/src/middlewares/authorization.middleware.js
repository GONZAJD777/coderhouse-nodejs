...
export const permission = async (ownership,object,user) => {
    try {
        ...
        if (ownProduct ==='owner') {
            if (product.owner != user.id && user.id != 'admin' ) throw new UnauthorizedError(errorCodes.ERROR_NOT_AUTHORIZED, errorMessages[errorCodes.ERROR_NOT_AUTHORIZED]); 
        } else if (ownProduct ==='notOwner') {
            if (product.owner == user.id && user.id != 'admin') throw new UnauthorizedError(errorCodes.ERROR_NOT_AUTHORIZED, errorMessages[errorCodes.ERROR_NOT_AUTHORIZED]); 
        }

        if (ownUser ==='owner') {
            if (object.user != user.id && user.id != 'admin' ) throw new UnauthorizedError(errorCodes.ERROR_NOT_AUTHORIZED, errorMessages[errorCodes.ERROR_NOT_AUTHORIZED]); 
